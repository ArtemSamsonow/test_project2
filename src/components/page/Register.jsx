        ...
        <section className="auth register">
            ...
            <div className="auth__login">
                ...
                <form className="login__form" name="auth__register" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <Input name='email' type='email' validate={register('email', EMAIL_VALIDATOR)} label='Email'
                           placeholder='Cash2000@bk.ru' errors={errors.email?.message}/>

                    <Input name='password' type={isShowPassCheck ? 'text' : "password"}
                           validate={register('password', PASSWORD_VALIDATOR)}
                           label='Password' placeholder='*****************' errors={errors.password?.message}
                           child={(
                    ...
                    <Input name='confirmPassword' errors={errors.confirmPassword?.message}
                           type={isShowPassCheck ? 'text' : "password"} validate={register('confirmPassword', {
                        PASSWORD_VALIDATOR, validate: (fieldValue) => {
                            return fieldValue === watch('password') || passwordCheckError
                        }
                    })} label='Confirmation password' placeholder='*****************'
                           child={(
                               <span className={isShowPassCheck ? "show-icon show" : 'show-icon no-show'}
                                     onClick={() => setIsShowPassCheck(!isShowPassCheck)}></span>
                           )}
                    ...
                    <Button type={"submit"} size={"max"} children={"Sign Up"}/>
                    {error ?
                        <span className="form__item-error error__request">Ошибка регистрации: {error}</span> : ''
                    }
                </form>
